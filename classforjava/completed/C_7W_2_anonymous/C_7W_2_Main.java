...
//        1. (1단계) 익명 클래스 : 객체 선언 시 오버라이드를 해서 재정의 된 임시 객체를 선언하는 것. 임시 객체이다 보니 이름이 없어 이름을 출력하면
//        출력하는 클래스의 이름이 나온다. (단점) 그리고 임시 객체여서 만들어진 내용을 반복 사용하기 위해서는 반복 선언이 필요함. 따라서 오버라이딩 하는
//        메소드가 1, 2개 인 경우 주로 사용. 그리고 ★ 일반적으로 인터페이스를 객체화 할 경우에 사용 왜냐하면 인터페이스는 객체화를 할 수 없으나
