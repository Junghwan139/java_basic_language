...
알파벳이 담긴 배열 spell과 외계어 사전 dic이 매개변수로 주어집니다. spell에 담긴 알파벳을 한번씩만
모두 사용한 단어가 dic에 존재한다면 1, 존재하지 않는다면 2를 return하도록 solution 함수를 완성해주세요.
...
*/
        ...
        for(String a : dic){
            if(a.length()==spell.length){
                for(int i = 0;i<spell.length;i++){
                    if(a.indexOf(spell[i])==-1){
                        answer = 2;
                        break;
                    }else{
                        answer = 1;
